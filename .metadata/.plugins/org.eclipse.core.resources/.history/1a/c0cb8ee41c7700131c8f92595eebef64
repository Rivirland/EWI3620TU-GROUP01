...
public class Main extends Frame implements GLEventListener, MouseListener, KeyListener, MouseMotionListener {
	...
	public void init(GLAutoDrawable drawable) {
		...
		// mazerunner = new MazeRunner(screenWidth, screenHeight, canvas,
		// drawable, gl, glu, userinput);
	...
	public void display(GLAutoDrawable drawable) {
		...
		case LOADLEVEL:
			...
			if (currentstate != gamestate) {
				...
				try {
					leveleditor = new LevelEditor(gl, screenWidth, screenHeight, LevelEditorWorld.readWorld(filename));
				} catch (FileNotFoundException e) {
