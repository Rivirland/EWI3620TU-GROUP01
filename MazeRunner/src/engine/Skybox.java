...
public class Skybox {
	...
	public static void displaySkybox(GL gl, int facingdirection) {
		GLU glu = new GLU();
		gl.glLoadIdentity();
		gl.glPushMatrix();
	    ...
	    gl.glLoadIdentity();
//	    
	    double vrpSkyBoxX = -Math.sin( Math.PI * (MazeRunner.player.getHorAngle()+90*facingdirection) / 180 ) * Math.cos( Math.PI * MazeRunner.player.getVerAngle() / 180 );
	    double vrpSkyBoxY = Math.sin( Math.PI * MazeRunner.player.getVerAngle() / 180 );
	    double vrpSkyBoxZ = -Math.cos( Math.PI * (MazeRunner.player.getHorAngle()+90*facingdirection) / 180 ) * Math.cos( Math.PI * MazeRunner.player.getVerAngle() / 180 );
...
//	    glu.gluLookAt(0, 0, 0,portalcamera.getVrpX(),portalcamera.getVrpY(), portalcamera.getVrpZ(), 0,1,0);
	    glu.gluLookAt(0, 0, 0,vrpSkyBoxX,vrpSkyBoxY, vrpSkyBoxZ, 0,1,0);
