...
public class Portal {
	...
	/*
	 * public Player toteleport (Player player, boolean teleportation){ if
	 * (teleportation){
	 * 
	 * } }
	 */
	...
	public void createCamera(GLUT glut, GL gl) {
		if (connected) {
			...
			// camera van een portal moet altijd in de richting van de portal
			// wijzen
			// +1/(Math.tan(dy/dx))?
			// de hoek is theta= tan-1(yd/xd)
			// gl.glRotatef((float)
			// ((float)(toportal.getFacingdirection()*90)+1/(Math.tan(dz/dx))),
			// 0, 1, 0);

			// float veclength= (float) Math.sqrt(dx*dx+dy*dy+dz*dz);
			/*
			 * if (facingdirection == 0 || facingdirection ==2){
			 * gl.glRotatef((float) (facingdirection*90+ Math.tan(dz/dx)), 0, 1,
			 * 0); } else {gl.glRotatef((float) (facingdirection*90+
			 * Math.tan(dx/dz)), 0, 1, 0);}
			 */
			// float straightlength= (float) Math.sqrt(dx*dx+dy*dy+dz*dz);
			// gl.glRotatef((float) (facingdirection*90+
			// Math.cos(straightlength/veclength)), 0, 1, 0);
