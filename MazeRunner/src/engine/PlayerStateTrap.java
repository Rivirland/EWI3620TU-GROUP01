...
public class PlayerStateTrap extends PlayerState {
	...
	public void itemUse() {
		if (Player.nrOfTraps > 0) {
			...
			double trapY = MazeRunner.level.getMaze(
					MazeRunner.level.getCurrentMaze(MazeRunner.player))
					.getMazeY();
			double trapZ = MazeRunner.player.getLocationZ();

			TrapDropped trapDropped = new TrapDropped(trapX, trapY, trapZ,
					MazeRunner.level.getCurrentMaze(MazeRunner.player));
			MazeRunner.visibleObjects.add(trapDropped);
			MazeRunner.level.getMaze(MazeRunner.level
					.getCurrentMaze(MazeRunner.player)).itemList
					.add(trapDropped);
