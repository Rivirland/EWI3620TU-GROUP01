...
public class LevelEditor extends Frame implements GLEventListener, MouseListener {
	...
	private void drawButtons(GL gl) {
		...
		gl.glColor3f(0.4f, 0.3f, 0.3f);
		boxOnScreen(gl, screenWidth-550f/600f*screenHeight, 50f/600f*screenHeight, 500f/600f*screenHeight);
		...
		//lineOnScreen(gl, 2 * buttonSize + 8.0f, screenHeight - buttonSize + 8.0f,
		//		2.5f * buttonSize, screenHeight -  8.0f);
		//lineOnScreen(gl, 2.5f * buttonSize, screenHeight -  8.0f,
		//		3 * buttonSize - 8.0f, screenHeight - buttonSize + 8.0f);
	}
	...
	private void drawFigure(GL gl) {
		...
		case DM_KOCH:
			if (points.size() >= 3) { //change to 2 for one line koch
				// If the draw mode is "koch" and the user has supplied at least
				// two points, draw a koch curve between those points
				int n = 6;
				p1 = points.get(0);
				p2 = points.get(1);
				p3 = points.get(2); //remove this for one line koch
				kochcurve(gl, p1.x, p1.y, p2.x, p2.y,n);
				kochcurve(gl, p2.x, p2.y, p3.x, p3.y,n); //remove this for one line koch
				kochcurve(gl, p3.x, p3.y, p1.x, p1.y,n); //remove this for one line koch
				}
			break;
		}
	}
	...
	/**
	 * Help method that returns the distance between 2 points.
	 */
	private float afstandtussen2punten(float x1, float y1, float x2, float y2){
		float afstand = (float) Math.sqrt((y2-y1)*(y2-y1)+(x2-x1)*(x2-x1));
		return afstand;
	}
	
	/**
	 * Help method that uses GL calls to draw a koch curve.
	 */
	private void kochcurve(GL gl, float x1, float y1, float x5, float y5,int order) {
		if(order>0){
		float x2 = (float) (x1+(x5-x1)/3);
		float y2 = (float) (y1+(y5-y1)/3);
		
	    
		float x3 = (float) (x1+(Math.sqrt(3)/3*afstandtussen2punten(x1,y1,x5,y5)*Math.cos(Math.atan2((y5-y1),(x5-x1))-Math.PI/6)));
		float y3 = (float) (y1+(Math.sqrt(3)/3*afstandtussen2punten(x1,y1,x5,y5)*Math.sin(Math.atan2((y5-y1),(x5-x1))-Math.PI/6)));
		
		float x4 = (float) (x1+2*(x5-x1)/3);
		float y4 = (float) (y1+2*(y5-y1)/3);
		
		kochcurve(gl, x1, y1, x2, y2,order-1);
		kochcurve(gl, x2, y2, x3, y3,order-1);
		kochcurve(gl, x3, y3, x4, y4,order-1);
		kochcurve(gl, x4, y4, x5, y5,order-1);
		
		}
		else{
			lineOnScreen(gl, x1, y1, x5, y5);
		}
	}
	...
	public void mouseReleased(MouseEvent me) {
		...
		if (!buttonPressed) {
			...
			if (drawMode == DM_POINT && points.size() >= 1) {
				...
				points.clear();
			} else if (drawMode == DM_KOCH && points.size() >= 3) { //change to 2 for one line koch
				// If we're drawing koch curves and two points were already stored, reset the points list
				points.clear();
			}
